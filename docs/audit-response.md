...
| W4, F11 | No synthetic production fallback; fresh attempts, strict exit/JSON/completeness validation, explicit failed-image accounting, cancellation/timeout/restart cleanup. | Real detector smoke plus malformed/stale/partial/failure fixtures. Companion detector exit-status bug remains; wrapper validates complete output independently. |
...
| I7, S1–S6 | Document companion exit, crop identity/indexing, prediction, metadata and boxing contracts requiring coordinated work. | **Deferred**, not repaired here. No complete five-stage runner or scientific accuracy claim. |
